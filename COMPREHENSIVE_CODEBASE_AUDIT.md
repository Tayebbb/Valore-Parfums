...
const [byUserIdSnap, byEmailSnap] = await Promise.all([
  ...
  db.collection(Collections.orders).where("customerEmail", "==", user.email).get() // 1 read
]);
...
for (let i = 0; i < ids.length; i += chunkSize) {
  ...
  const snapshots = await db.collection("perfumes")
...
for (const order of orders) {
  const items = await db.collection("orders").doc(order.id).collection("items").get()
}
// Cost: 2 + N reads
...
const nextConfig = {
  ...
  async headers() {
    return [
      {
        source: "/(.*)\\.(js|css|...svg)$",
        headers: [{ key: "Cache-Control", value: "public, max-age=31536000, immutable, stale-while-revalidate=86400" }]
      },
      {
        // Images: 30 days
